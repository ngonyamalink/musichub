<?php

/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
class User_lib
{

    public $user_id;

    public $name;

    public $surname;

    public $username;

    public $password;

    public $province_id;

    public function get_user_by_username_password($u, $p)
    {
        $CI = &get_instance();

        $CI->load->model("user_db");

        $data = $CI->user_db->get_user_by_username_password($u, $p);
        if ($data) {
            $this->user_id = $data['user_id'];
            $this->name = $data['name'];
            $this->surname = $data['surname'];
            $this->username = $data['username'];
            $this->password = $data['password'];
            $this->province_id = $data['province_id'];
            return true;
        } else {
            return false;
        }
    }

    public function get_user_by_authkey($authstring)
    {
        $CI = &get_instance();
        $CI->load->model("user_db");

        $data = $CI->user_db->get_user_by_authkey($authstring);

        if ($data) {
            $this->user_id = $data['member_id'];
            $this->name = $data['member_name'];
            $this->surname = $data['member_surname'];
            $this->username = $data['member_email'];
            $this->password = $data['member_password'];
            $this->province_id = $data['member_province_id'];
            return true;
        } else {
            return false;
        }
    }
}